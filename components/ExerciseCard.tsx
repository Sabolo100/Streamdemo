...
export default function ExerciseCard({
  ...
  const focusProfile = getExerciseFocusProfile(item.video, language);

  return (
    <article className="rounded-[20px] border border-[rgba(23,33,30,0.08)] bg-white/78 p-3.5 shadow-sm">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-2">
            <span className="inline-flex h-8 w-8 items-center justify-center rounded-full bg-[#17211e] text-sm font-semibold text-white">
              {item.order}
            </span>
            <div>
              <p className="text-base font-semibold text-[#17211e]">
          ...
          <div className="mt-2.5 flex flex-wrap gap-2">
            {item.video.tags.slice(0, 5).map((tag) => (
              <span
                className="rounded-full border border-[rgba(23,33,30,0.08)] bg-[rgba(23,33,30,0.04)] px-3 py-1 text-xs font-medium text-[#5a655f]"
                key={tag}
              >
        ...
        <div className="flex w-full flex-col items-start gap-2 lg:w-auto lg:min-w-[240px] lg:items-end">
      ...
      <div className="mt-3 grid gap-2.5 rounded-[16px] bg-[rgba(23,33,30,0.03)] p-3 md:grid-cols-[1.2fr_1fr]">
        <div>
          <p className="text-xs font-semibold uppercase tracking-[0.22em] text-[#5a655f]">
            {copy.prescription}
          </p>
          <p className="mt-1.5 text-sm font-semibold text-[#17211e]">{item.prescription.summary}</p>
          {item.prescription.loadGuidance ? (
            <p className="mt-1 text-sm text-[#5a655f]">{item.prescription.loadGuidance}</p>
          ) : null}
        </div>
      ...
      <div className="mt-3 flex flex-wrap items-center gap-3 text-sm text-[#5a655f]">
        <span>
